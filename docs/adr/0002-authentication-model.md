...
- ADR-0004 — the frontend state split: server state in TanStack Query, UI state in Zustand, the URL
  for shareable state, and nothing personal persisted. Its §7.6 makes the logout step in §5 above
  normative (`queryClient.clear()` plus a reset of every Zustand store).
